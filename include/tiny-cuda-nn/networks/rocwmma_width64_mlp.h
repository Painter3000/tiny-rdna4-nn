...
private:
	struct ForwardContext final : Context {
		GPUMatrixDynamic<float> pre_activation_0;
		GPUMatrixDynamic<__half> hidden_0;
		GPUMatrixDynamic<float> pre_activation_1;
		GPUMatrixDynamic<__half> hidden_1;
		GPUMatrixDynamic<__half> owned_output;
	};

	GPUMemory<__half> m_forward_params;
	GPUMemory<__half> m_forward_inference_params;
};
...
struct Phase4A2StalenessAttestation {
	uint64_t logical_weight_mutation_count;
	uint64_t forward_view_refresh_count;
	uint64_t fused_forward_launch_count;
	uint64_t logical_weight_version;
	uint64_t packed_view_version;
	bool forward_view_is_current;
	bool stale_launch_rejected_before_kernel;
};
...
struct Phase4A2RuntimeAttestation {
	uint64_t forward_view_refresh_count;
	uint64_t fused_forward_launch_count;
	uint64_t logical_weight_version;
	uint64_t packed_view_version;
