...
SMOKE_REPORT = ROOT / "phase3b1_reports/phase3b1f0a_harness_smoke.json"
...
def validate_record(record, case, contract):
    ...
    return {
        ...
        "algorithm_contract": (
            bool(evidence.get("algorithm_ids")) if gemm_expected
        ...
        "pair_identity": (
            ...
            and identity.get("fp32_master_parameter_sha256") == identity.get("candidate_master_parameter_sha256")
        ...
        "numerical": all(name in numerical for name in ("output", "dinput", "network_gradient", "encoding_gradient"))
                     and all(item.get("nan_count") == 0 and item.get("inf_count") == 0 for item in numerical.values()),
...
def smoke():
    ...
    pre_returncode = correctness_snapshot(pre_path)
    correctness_pre = json.loads(pre_path.read_text()) if pre_path.exists() else {"passed": False}
    records = []
    ...
    post_returncode = correctness_snapshot(post_path)
    correctness_post = json.loads(post_path.read_text()) if post_path.exists() else {"passed": False}
    static = static_audit(manifest, contract)
    all_checks = [value for item in records for value in item["checks"].values()]
    manipulations = run_smoke_manipulations()
    decision = "PHASE3B1F0A_HARNESS_READY" if (
        all(static.values()) and all(all_checks) and all(x["passed"] for x in manipulations)
        and pre_returncode == 0 and post_returncode == 0
        and correctness_pre.get("passed") is True and correctness_post.get("passed") is True
    ) else "PHASE3B1F0A_BLOCKED"
    report = {
    ...
    print(decision)
    return 0 if decision == "PHASE3B1F0A_HARNESS_READY" else 2
