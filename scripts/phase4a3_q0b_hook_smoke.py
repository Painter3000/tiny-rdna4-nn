#!/usr/bin/env python3
"""Fail-fast smoke for the Q0b test-only native benchmark hook."""
...
def main() -> int:
    ...
    if not hasattr(_C.Module, "phase4a3_q0b_benchmark_inference"):
        raise RuntimeError("Q0b test-only native hook is missing")
    ...
    print(f"PHASE4A3_Q0B_API_BATCH_GRANULARITY: {api_granularity}")
    print("PHASE4A3_Q0B_NATIVE_HOOK_SMOKE: PASS")
