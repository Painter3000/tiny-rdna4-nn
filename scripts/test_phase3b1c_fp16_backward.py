...
TCNN_RDNA4_P3B1C1A_FINAL_AUDIT_001 derives counts from executed records.
"""
...
def main():
 ...
 allcases=cases+rectangles+adversarial+modes
 supplemental_checks=[{"name":"double_accumulate","passed":bool(acc_ok)},{"name":"dynamic_batches","passed":bool(dynamic_ok)},{"name":"multistream","passed":bool(multi_ok)},{"name":"event_chain","passed":bool(event_ok)},{"name":"dinput_not_requested","passed":bool(no_dinput_ok)},{"name":"static_loss_scaling","passed":bool(scale_ok)},{"name":"inf_propagation_and_fp16_underflow_quantization","passed":bool(inf_underflow_ok)},{"name":"finite_to_fp16_parameter_gradient_overflow","passed":bool(finite_overflow_ok)},{"name":"estimated_scratch_returns_to_zero","passed":after["scratch_bytes_live"]==0}]
 functional_cases=len(allcases)+len(dz_cases)+len(supplemental_checks);passed_cases=sum(c.get("passed") is True for c in allcases)+sum(c.get("passed") is True for c in dz_cases)+sum(c["passed"] is True for c in supplemental_checks)
 decision="PROCEED_TO_3B1D" if passed_cases==functional_cases else "PHASE3B1C_BLOCKED"
 direct_masks=sum(c["mask_mismatches"] for c in dz_cases)
 doc={"marker":MARKER,"audit_hardening_marker":"TCNN_RDNA4_P3B1C1_BACKWARD_AUDIT_HARDENING_001","final_audit_marker":"TCNN_RDNA4_P3B1C1A_FINAL_AUDIT_001","base_commit":BASE,"decision":decision,"tolerances_frozen_before_final_matrix":TOL,"near_zero":NEAR_ZERO,"functional_cases":functional_cases,"passed_cases":passed_cases,"maxima":{"dx":maxcat(allcases,"dx"),"dw":maxcat([c for c in allcases if c["mode"]!="Ignore"],"dw"),"db":maxcat([c for c in allcases if c["mode"]!="Ignore"],"db"),"dz":maxcat(dz_cases,"dz")},"direct_dz_mask_mismatches":direct_masks,"integrated_relu_mask_validation":"indirect_through_dx_dw_db_oracles","gradient_modes":{"cases":modes,"overwrite_passed":modes[0]["passed"],"accumulate_passed":modes[1]["passed"],"ignore_passed":modes[2]["passed"],"double_accumulate":bool(acc_ok)},"direct_dz_db_oracle":dz_cases,"supplemental_checks":supplemental_checks,"event_chained_cross_stream":{"passed":event_ok},"dinput_not_requested":{"passed":no_dinput_ok},"loss_scaling":{"dynamic":"not implemented","static_factor":8,"dx":scale_dx,"dparams":scale_dp,"passed":scale_ok},"range_tests":{"inf_propagation_and_fp16_underflow_quantization":{"passed":inf_underflow_ok},"finite_to_fp16_parameter_gradient_overflow":{"all_inputs_finite":finite_inputs,"dz_finite":bool(torch.isfinite(rdz).all()),"fp32_db_finite":bool(torch.isfinite(rdb).all()),"native_fp16_parameter_gradient_inf_detected":native_overflow,"post_overflow_followup_passed":followup_ok,"memory_corruption_detected_by_followup":not followup_ok,"passed":finite_overflow_ok}},"estimated_backward_scratch":{"semantics":"host_scope_estimate_not_event_bound_not_async_allocator_peak_not_multi_gpu","estimated_backward_scratch_live_bytes":after["scratch_bytes_live"],"estimated_backward_scratch_peak_bytes":after["scratch_bytes_peak"]},"dynamic_batches":{"sequence":seq,"first":c1,"second":c2,"pytorch_hip_memory":{"before_warmup":mem_before,"after_first_warmup":mem1,"after_second_pass":mem2},"passed":dynamic_ok},"multistream":{"rounds_per_pass":64,"before":before,"warm":warm,"after":after,"passed":multi_ok},"cases":allcases}
