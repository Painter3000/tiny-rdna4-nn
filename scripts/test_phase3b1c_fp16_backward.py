#!/usr/bin/env python3
"""TCNN_RDNA4_P3B1C_FP16_BACKWARD_001 production backward regression."""
...
def run_case(ni,nh,no,nl,batch,ha,oa,seed,adversarial=False,mode="Overwrite",stream=None):
 ...
 masks=sum(int(((acts[i]>0)!=(acts[i]>0)).sum()) for i in range(len(acts)))
 ...
 return {"shape":[ni,nh,no],"layers":nl,"batch":batch,"hidden_activation":ha,"output_activation":oa,"adversarial":adversarial,"mode":mode,"dx":mdx,"dparams":mdp,"dw":mw,"db":mb,"relu_mask_mismatches":masks,"passed":bool(ok)}
...
def main():
 ...
 sequence();c1=counts();mem1=torch.cuda.memory_allocated();sequence();c2=counts();mem2=torch.cuda.memory_allocated();stable_keys=("cache_misses","cache_size","heuristic_queries","execution_handle_creations","descriptor_count");dynamic_ok=all(c1[k]==c2[k] for k in stable_keys)
 ...
 ou=torch.tensor([[float("inf"),2**-25,-2**-25,1.]],device="cuda",dtype=torch.float16);oa=torch.ones_like(ou);odz,odb=_C._hipblaslt_fp16_test_activation_biasgrad(ou,oa,False);range_ok=bool(torch.isinf(odz[0,0]) and odz[0,1].item()==0 and odz[0,2].item()==0 and torch.isinf(odb[0]))
 allcases=cases+rectangles+adversarial+modes;extras_ok=event_ok and no_dinput_ok and scale_ok and range_ok;decision="PROCEED_TO_3B1D" if all(c["passed"] for c in allcases) and all(c["passed"] for c in dz_cases) and acc_ok and dynamic_ok and multi_ok and extras_ok and after["scratch_bytes_live"]==0 else "PHASE3B1C_BLOCKED"
 doc={"marker":MARKER,"base_commit":BASE,"decision":decision,"tolerances_frozen_before_final_matrix":TOL,"near_zero":NEAR_ZERO,"functional_cases":len(allcases)+len(dz_cases)+4,"passed_cases":sum(c["passed"] for c in allcases)+sum(c["passed"] for c in dz_cases)+sum((event_ok,no_dinput_ok,scale_ok,range_ok)),"maxima":{"dx":maxcat(allcases,"dx"),"dw":maxcat([c for c in allcases if c["mode"]!="Ignore"],"dw"),"db":maxcat([c for c in allcases if c["mode"]!="Ignore"],"db"),"dz":maxcat(dz_cases,"dz")},"relu_mask_mismatches":sum(c["relu_mask_mismatches"] for c in allcases)+sum(c["mask_mismatches"] for c in dz_cases),"gradient_modes":{"cases":modes,"double_accumulate":bool(acc_ok)},"direct_dz_db_oracle":dz_cases,"event_chained_cross_stream":{"passed":event_ok},"dinput_not_requested":{"passed":no_dinput_ok},"loss_scaling":{"dynamic":"not implemented","static_factor":8,"dx":scale_dx,"dparams":scale_dp,"passed":scale_ok,"overflow_underflow_explicit":range_ok},"dynamic_batches":{"sequence":seq,"first":c1,"second":c2,"memory":[mem1,mem2],"passed":dynamic_ok},"multistream":{"rounds_per_pass":64,"before":before,"warm":warm,"after":after,"passed":multi_ok},"cases":allcases}
 a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(doc,indent=2)+"\n");print(json.dumps({k:doc[k] for k in ("decision","functional_cases","passed_cases","maxima","relu_mask_mismatches")}));return 0 if decision=="PROCEED_TO_3B1D" else 1
