...
## Public validation and demo

The public command surface has exactly three fail-closed commands:
...
Tier 1 is portable self-contained validation on this concrete host. Tier 2 is
an informative comparison with frozen, environment-bound reference anchors.
Tier 3 requires validation by an independent foreign host. A local Tier-1 PASS
is not Tier 3 and does not prove universal ROCm/GPU compatibility. See
[the public validation contract](docs/VALIDATION.md).

Native binaries retained in freeze material are reference and provenance
artifacts, not a universal binary distribution. Rebuild modules locally from
the included sources and build contracts.

## Radeon AI PRO R9700 portable validation

Run the public fresh-clone correctness smoke with one command:

```bash
./scripts/fresh_clone_user_smoke.sh
```

The Phase-3D-A3 qualification applies to the frozen reference build and is not
transitive to locally rebuilt binaries. A foreign build is validated for its
local environment exclusively by its own Tier-1 smoke. Tier-2 equality with the
reference S100 state is informational and is not a required gate. See
[`docs/PHASE3C_PORTABLE_SMOKE.md`](docs/PHASE3C_PORTABLE_SMOKE.md) for details.
